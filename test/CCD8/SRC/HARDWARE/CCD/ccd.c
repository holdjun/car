...
void DT(void)
{
	uint8_t i,value_max,value_min,j;
	value_max=P1[2];
	j=0;
	for(i=2;i<126;i++) 
  {
...
//	if(j>=4&&j<=123)
//	{
//		value_max=(P1[j-2]+P1[j-1]+P1[j]+P1[j+1]+P1[j+2])/5;
//	}
//	if(value_min<=80)
//	{
//		value_min=80;
...
void LB(void)
{
  ...
  for(i=3;i<125;i++)
  {
    if(P1[i]==0&&P1[i]!=P1[i-1]&&P1[i]!=P1[i+1])
      P1[i]=255;
    else if(P1[i]==250&&P1[i]!=P1[i-1]&&P1[i]!=P1[i+1])
      P1[i]=0;
	}
  for(i=3;i<124;i++)
  {
...
/*********************************************************************/	
	...
	if(Center <= 63)
	{
		if (63 - Center <= 7)
		{
			FTMDuty = 4400 - (63 - Center) * a1;
		}
		if (63 - Center <= 15 && 63 - Center > 7)
		{
			FTMDuty = 4400 - (63 - Center) * a2;
		}
		if (63 - Center <= 25 && 63 - Center > 15)
		{
			FTMDuty = 4400 - (63 - Center) * a3;
		}
		if (63 - Center <= 40 && 63 - Center > 25)
		{
			FTMDuty = 4400 - (63 - Center) * a4;		
		}
		if (63 - Center > 40)
		{
			FTMDuty = 4400 - (63 - Center) * a5;		
		}
//		if (63 - Center <= 30 && 63 - Center > 25)
//		{
//			FTMDuty = 4400 - (63 - Center) * a6;		
//		}
//		if (63 - Center > 30)
//		{
//			FTMDuty = 4400 - (63 - Center) * a7;			
//		}

		FTM_PWM_ChangeDuty(FTM2_CH0_PA10,FTMDuty);
		if(FTMDuty <= 3700) FTMDuty = 3700;
		if((Right-Left)==Width&&Center>60)
		{
			FTM_PWM_ChangeDuty(FTM2_CH0_PA10,4400);
		}
	}
	if(Center >= 64)
	{
		if (Center - 64 <= 7)
		{
			FTMDuty = 4400 + (Center - 64) * a1;
		}
		if (Center - 64 <= 15 && Center - 64 > 7)
		{
			FTMDuty = 4400 + (Center - 64) * a2;
		}
		if (Center - 64 <= 25 && Center - 64 > 15)
		{
			FTMDuty = 4400 + (Center - 64) * a3;
		}
		if (Center - 64 <= 40 && Center - 64 > 25)
		{
			FTMDuty = 4400 + (Center - 64) * a4;
		}
		if (Center - 64 > 40)
		{
			FTMDuty = 4400 + (Center - 64) * a5;
		}
//		if (Center - 64 <= 30 && Center - 64 > 25)
//		{
//			FTMDuty = 4400 + (Center - 64) * a6;
//		}
//		if (Center - 64 > 30)
//		{
//			FTMDuty = 4400 + (Center - 64) * a7;
//		}

		FTM_PWM_ChangeDuty(FTM2_CH0_PA10,FTMDuty);
		if(FTMDuty >= 5100) FTMDuty = 5100;
		if((Right-Left)==Width&&Center<67)
		{
			FTM_PWM_ChangeDuty(FTM2_CH0_PA10,4400);
		}
	}
