...
void CCD_HeiXian(void)
{
	...
	for (i = 2;i < 126;i++) 
  {
	...
	/********************************类型********************************/
	if (((Left_last - Left >= 10 && Left_last - Left <= 23)|| (Right - Right_last >= 10 && Right - Right_last <= 23)) && Cross_flag == 0)
	{
		if (CenterLast[0] <= 63) Center = 35;
		if (CenterLast[0] >  63) Center = 91;
	}
	if (((CenterLast[0] - Center <= 70) && (CenterLast[0] - Center >= 30)) || ((Center - CenterLast[0] <= 70) && (Center - CenterLast[0] >= 30)))
	{
		Center = CenterLast[0];
	}
	...
	if ((Center <= 44 && Left <= 15) || (Center >=84 && Right >= 113))
	{
		leixin = 1;//wandao
		jiasuflag = 0;
	}
