...
uint16_t FTMDuty_1 = 2050;
uint16_t FTMDuty_2 = 1900;
uint16_t FTMDuty_3 = 550;
...
uint16_t black_Value = 80;
uint16_t white_Value = 150;
...
//				UART_Send_Con();
			...
			if(Key_right == 0) //上位机
			{
				...
				if(Key_right == 0)
				{
					...
					{
						OLED_Clear();
						OLED_Write_String(2,2,"SD writing");
						Sd_D();
						UART_Send_Con();
					}
				}
